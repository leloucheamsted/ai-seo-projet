...
export const ContentAnalysisSummaryRepository = {
    ...
    async getTasksByGroups(userId: number, page: number = 1, limit: number = 10) {
        const tasks = await ContentAnalysisSummaryTask.findAll({
        ...
        tasks.forEach(task => {
            ...
            const keyword = params?.keyword || 'unknown';
            const searchMode = params?.search_mode || 'unknown';
            const pageType = Array.isArray(params?.page_type) && params.page_type.length > 0 ? params.page_type[0] : 'unknown';

            // Créer une fenêtre de temps de 5 minutes
            const timeWindow = Math.floor(new Date(task.created_at!).getTime() / (5 * 60 * 1000));

            const groupKey = `${keyword}_${searchMode}_${pageType}_${timeWindow}`;
    ...
    async getTaskGroupById(userId: number, groupId: string) {
        ...
        const searchMode = parts[parts.length - 3];
        ...
        const filteredTasks = tasks.filter(task => {
            const params = task.params as any;
            const taskKeyword = params?.keyword || 'unknown';
            const taskSearchMode = params?.search_mode || 'unknown';
            const taskPageType = Array.isArray(params?.page_type) && params.page_type.length > 0 ? params.page_type[0] : 'unknown';

            return taskKeyword === keyword &&
                taskSearchMode === searchMode &&
                taskPageType === pageType;
        });
    ...
    async deleteTaskGroup(userId: number, groupId: string) {
        ...
        const deletedCount = await ContentAnalysisSummaryTask.destroy({
            where: {
                id: {
                    [Op.in]: taskIds
                },
